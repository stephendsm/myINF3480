...
Comm::Comm(std::string device_name, int baud_rate) {
  ...
  if (!_data->_port->openPort()) {
    throw std::runtime_error("Could not open serial port: '" + device_name +
                             "'");
  }
...
boost::optional<ModelID> Comm::ping(const ServoID id) const {
  ModelID result;
  uint8_t err;
  const auto ping_res = static_cast<Result>(
      _data->_packet->ping(_data->_port.get(), id, &result, &err));
  if (ping_res == Result::Success) {
    return result;
  } else {
    std::cerr
        << "\033[31m[Comm::ping] problem sending ping to servo with ID:\033[0m "
        << static_cast<int>(id) << ", error: " << ping_res << std::endl;
    std::cerr << "\033[31m[Comm::ping] additional error:\033[0m "
              << _data->_packet->getRxPacketError(err) << std::endl;
  }
  return {};
}
...
std::vector<std::pair<ServoID, ModelID>> Comm::scan() {
  ...
  const auto scan_res = static_cast<Result>(
      _data->_packet->broadcastPing(_data->_port.get(), ids));
  if (scan_res == Result::Success) {
    for (auto id : ids) {
      if (auto model = ping(id)) {
        ...
        std::cerr
            << "\033[31m[Comm::scan] problem pinging servo with ID:\033[0m "
            << static_cast<int>(id) << std::endl;
      }
    }
  } else {
    std::cerr << "\033[31m[Comm::scan] problem broadcasting ping:\033[0m "
              << scan_res << std::endl;
  }
  return res;
}

std::unique_ptr<GroupReader> Comm::make_reader(Addr addr,
                                               std::vector<ServoID> ids) {
  return std::unique_ptr<GroupReader>(
      new GroupReader(addr, std::move(ids), _data));
}
...
Result Comm::bulk_write(const std::vector<IdAddrVal> data) {
  ...
  for (auto update : data) {
    ...
    if (!writer.addParam(update.id, raw_addr, addr_size,
      ...
      std::cerr << "\033[31m[Comm::bulk_write] Could not write " << raw_addr
                << "=" << update.value << " [" << addr_size
                << "] for ID:\033[0m " << static_cast<int>(update.id)
                << std::endl;
    }
  }
...
GroupReader::GroupReader(const Addr addr, std::vector<ServoID> ids,
    ...
    : _addr(addr), _ids(std::move(ids)), _comm(std::move(comm)),
      _reader(_comm->_port.get(), _comm->_packet.get(),
              static_cast<uint16_t>(_addr),
              memsize[static_cast<uint16_t>(_addr)]) {
  ...
  if (send_res == Result::Success) {
    ...
    for (auto id : _ids) {
      if (_reader.isAvailable(id, raw_addr, memsize[raw_addr])) {
        ...
        std::cerr
            << "\033[31m[GroupReader::read_data] No data available for:\033[0m "
            << id << std::endl;
      }
    }
  } else {
    std::cerr << "\033[31m[GroupReader::read_data] Error reading from servos, "
                 "error:\033[0m "
              << send_res << std::endl;
  }
  return res;
...
MxBulkReader::MxBulkReader(std::vector<ServoID> ids,
                           std::shared_ptr<Comm::Data> comm)
  ...
  if (send_res == Result::Success) {
    ...
    for (auto id : _ids) {
      ...
      if (_reader.isAvailable(id, pwm_addr, memsize[pwm_addr])) {
        ...
        std::cerr << "\033[31m[MxBulkReader::read_data] PWM not available for "
                     "servo:\033[0m "
                  << static_cast<int>(id) << std::endl;
      }
      ...
      if (_reader.isAvailable(id, current_addr, memsize[current_addr])) {
        ...
        std::cerr << "\033[31m[MxBulkReader::read_data] Current not available "
                     "for servo:\033[0m "
                  << static_cast<int>(id) << std::endl;
      }
      const uint16_t velocity_addr =
          static_cast<uint16_t>(Addr::PresentVelocity);
      if (_reader.isAvailable(id, velocity_addr, memsize[velocity_addr])) {
        res.velocity =
            _reader.getData(id, velocity_addr, memsize[velocity_addr]);
      } else {
        std::cerr << "\033[31m[MxBulkReader::read_data] Velocity not available "
                     "for servo:\033[0m "
                  << static_cast<int>(id) << std::endl;
      }
      const uint16_t position_addr =
          static_cast<uint16_t>(Addr::PresentPosition);
      if (_reader.isAvailable(id, position_addr, memsize[position_addr])) {
        res.position =
            _reader.getData(id, position_addr, memsize[position_addr]);
      } else {
        std::cerr << "\033[31m[MxBulkReader::read_data] Position not available "
                     "for servo:\033[0m "
                  << static_cast<int>(id) << std::endl;
      }
      const uint16_t input_voltage_addr =
          static_cast<uint16_t>(Addr::PresentVoltage);
      if (_reader.isAvailable(id, input_voltage_addr,
                              memsize[input_voltage_addr])) {
        res.input_voltage = _reader.getData(id, input_voltage_addr,
                                            memsize[input_voltage_addr]);
      } else {
        std::cerr << "\033[31m[MxBulkReader::read_data] Voltage not available "
                     "for servo:\033[0m "
                  << static_cast<int>(id) << std::endl;
      }
      const uint16_t temperature_addr =
          static_cast<uint16_t>(Addr::PresentTemperature);
      if (_reader.isAvailable(id, temperature_addr,
                              memsize[temperature_addr])) {
        res.temperature =
            _reader.getData(id, temperature_addr, memsize[temperature_addr]);
      } else {
        std::cerr << "\033[31m[MxBulkReader::read_data] Temperature not "
                     "available for servo:\033[0m "
                  << static_cast<int>(id) << std::endl;
      }
      result.push_back(res);
    }
  } else {
    std::cerr << "\033[31m[MxBulkReader::read_data] Error reading bulk data, "
                 "error:\033[0m "
              << send_res << std::endl;
  }
  return result;
...
double from_temp(const uint8_t temp) { return static_cast<double>(temp); }
