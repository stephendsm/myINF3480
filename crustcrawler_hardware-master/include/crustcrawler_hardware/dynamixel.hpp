...
enum class Addr : uint16_t {
  ...
  ReturnDelay = 9, // Time between reception of instruction packet and return of
                   // status packet
  OperatingMode = 11,
  InternalMaxTemp = 31,
  MaxTorque = 38,
...
private:
  GroupReader(const Addr addr, std::vector<ServoID> ids,
...
private:
  ...
  static constexpr uint16_t _start_addr =
      static_cast<uint16_t>(Addr::PresentPWM);
